...
function getStoryMessageContent(case_id, story_id, delay) {
    ...
    $.ajax({
        ...
        success: function (data) {
			...
			if(data['message_type']=='pdf'){
 			 	 //PDF
                        content.append("<embed src=" + data['message_url'] + " class='embed-pdf' alt='details' pluginspage='http://www.adobe.com/products/acrobat/readstep2.html'><a href='"+ data['message_url'] +"' target='_blank'> Descargar </a>")
			}
			else if(data['message_type']=='html'){
				//HTML
				content.append(data['story_text'])
			}
			else if(data['message_type']=='audio'){
				//audio
				content.append("<audio controls><source src='"+data['message_url']+"'></<audio>")
			}
			else if(data['message_type']=='movie'){
				//movie
				content.append("<video controls><source src='"+data['message_url']+"'></<video>")
			}
                    } else {
